# MOV data analysis for Liberia for measles 
# Author: Caroline Nondin
# Objective: Understand if high performing countries such as Liberia 
#successfully addressed “missed opportunities” for measles, and which factor are 
...
#these include: mea1_missed_opportunity (outcome), wom_agecat, edu, female_head, literate, total_children_born, wom_occ, marital, hhsize, assets, urban, strata
dt_Lib <- select(dt_Lib, mea1_missed_opportunity, wom_agecat, edu, female_head,
                 literate, total_children_born, wom_occ, marital, hhsize, assets, urban, strata)

################## ######## Analysis Liberia ##################################

#logistic regression Liberia:  
mylogit_Lib <- glm(mea1_missed_opportunity ~ wom_agecat + edu + female_head + literate +
...
mylogit_Lib_2013 <- glm(mea1_missed_opportunity ~ wom_agecat + edu + female_head + literate +
                          total_children_born + wom_occ + marital + hhsize + assets + 
...
#log reg on stratified data, 2019
df_2019 = dt_Lib[dt_Lib$strata == '2019',]

mylogit_Lib_2019 <- glm(mea1_missed_opportunity ~ wom_agecat + edu + female_head + literate +
...
pred_prob_lit <- with(dt_Lib, data.frame(edu = factor(rep(c("No education"), each= 100)),
...
pred_prob_asset <- with(dt_Lib, data.frame(edu = factor(rep(c("No education"), each= 100)),
                                               wom_agecat = factor(rep(c("20-34"), each = 100)), female_head = factor(rep(c("Female"), each =100)),
                                               literate = factor(rep(c("Iliterate"), each = 100)), total_children_born = factor(rep(c("2-3 children"), each = 100)),
                                               wom_occ = factor(rep(c("Employed"), each =100)), marital = factor(rep(c("Married"), each = 100)), hhsize = mean(hhsize), 
                                               assets = factor(rep(unique(dt_Lib$assets), each = 100)), urban = factor(rep(c("Rural household"), each = 100)),
                                               strata = factor(rep(unique(dt_Lib$strata), each = 50, length.out = 100))))

pred_prob_urban <- with(dt_Lib, data.frame(edu = factor(rep(c("No education"), each= 100)),
                                             wom_agecat = factor(rep(c("20-34"), each = 100)), female_head = factor(rep(c("Female"), each =100)),
                                             literate = factor(rep(c("Iliterate"), each = 100)), total_children_born = factor(rep(c("2-3 children"), each = 100)),
                                             wom_occ = factor(rep(c("Employed"), each =100)), marital = factor(rep(c("Married"), each = 100)), hhsize = mean(hhsize), 
                                             assets = factor(rep(c("Quintile 1"), each = 100)), urban = factor(rep(unique(dt_Lib$urban), length.out = 100)),
                                             strata = factor(rep(unique(dt_Lib$strata), each = 50, length.out = 100))))

#calculating the predicted probs and the standard errors: 
pred_prob_lit <- cbind(pred_prob_lit, predict(mylogit_Lib, newdata = pred_prob_lit, type = "link",
                                              se = TRUE))
...
pred_prob_asset <- cbind(pred_prob_asset, predict(mylogit_Lib, newdata = pred_prob_asset, type = "link",
                                                          se = TRUE))
pred_prob_asset <- within(pred_prob_asset, {
  PredictedProb <- plogis(fit)
  LL <- plogis(fit - (1.96 * se.fit))
  UL <- plogis(fit + (1.96 * se.fit))
})
head(pred_prob_asset)


pred_prob_urban <- cbind(pred_prob_urban, predict(mylogit_Lib, newdata = pred_prob_urban, type = "link",
                                                      se = TRUE))
pred_prob_urban <- within(pred_prob_urban, {
  PredictedProb <- plogis(fit)
  LL <- plogis(fit - (1.96 * se.fit))
  UL <- plogis(fit + (1.96 * se.fit))
})
head(pred_prob_urban)
...
p_asset <- ggplot(pred_prob_asset, aes(x= assets, y=PredictedProb, fill=strata)) + 
  geom_bar(stat="identity", position=position_dodge()) +
  geom_errorbar(aes(ymin=LL, ymax=UL), width=.2,
                position=position_dodge(.9))

p_asset + scale_fill_brewer(palette="Paired") + theme_minimal()

p_urban <- ggplot(pred_prob_urban, aes(x=urban, y=PredictedProb, fill=strata)) + 
  geom_bar(stat="identity", position=position_dodge()) +
  geom_errorbar(aes(ymin=LL, ymax=UL), width=.2,
                position=position_dodge(.9))

p_urban + scale_fill_brewer(palette="Paired") + theme_minimal()
