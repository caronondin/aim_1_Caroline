# MOV data analysis for Nigeria for measles 
# Author: Caroline Nondin
# Objective: Understand if high performing countries such as Nigeria 
#successfully addressed “missed opportunities” for measles and identify which factors 
...
dt_Nig <- select(dt_Nig, mea1_missed_opportunity, wom_agecat, edu, female_head,
                 literate, total_children_born, wom_occ, marital, hhsize, assets, urban, strata)

#logistic regression Nigeria: 
mylogit_Nig <- glm(mea1_missed_opportunity ~ wom_agecat + edu + female_head + literate +
...
mylogit_Nig_2013 <- glm(mea1_missed_opportunity ~ wom_agecat + edu + female_head + literate +
                          total_children_born + wom_occ + marital + hhsize + assets + 
...
mylogit_Nig_2018 <- glm(mea1_missed_opportunity ~ wom_agecat + edu + female_head + literate +
...
pred_prob_rural <- with(dt_Nig, data.frame(edu = factor(rep(c("No education"), each = 100)),
                                            wom_agecat = factor(rep(c("20-34"), each = 100)), female_head = factor(rep(c("Female"), each =100)),
                                            literate = factor(rep(c("Iliterate"), each = 100)), total_children_born = factor(rep(c("2-3 children"), each = 100)),
                                            wom_occ = factor(rep(c("Employed"), each =100)), marital = factor(rep(c("Married"), each = 100)), hhsize = mean(hhsize), 
                                            assets = factor(rep(c("Quintile 2"), each = 100)), urban = factor(rep(unique(dt_Nig$urban), length.out = 100)),
                                            strata = factor(rep(unique(dt_Nig$strata), each = 50, length.out = 100))))
...
pred_prob_rural <- cbind(pred_prob_rural, predict(mylogit_Nig, newdata = pred_prob_rural, type = "link",
                                              se = TRUE))
pred_prob_rural <- within(pred_prob_rural, {
  PredictedProb <- plogis(fit)
  LL <- plogis(fit - (1.96 * se.fit))
  UL <- plogis(fit + (1.96 * se.fit))
})
head(pred_prob_rural)
...
p_rural <- ggplot(pred_prob_rural, aes(x=urban, y=PredictedProb, fill=strata)) + 
  geom_bar(stat="identity", position=position_dodge()) +
  geom_errorbar(aes(ymin=LL, ymax=UL), width=.2,
                position=position_dodge(.9))

p_rural + scale_fill_brewer(palette="Paired") + theme_minimal()
